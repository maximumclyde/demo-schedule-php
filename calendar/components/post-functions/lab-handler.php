...
if(isset($_POST['submit-lab'])){
    ...
    if($result){
        ...
        //$sql = "insert into subjectdb (Activity_ID, Prof_ID, Subject_name) values($id, ".$_SESSION['userid'].", '".$class['title']."');";
        ...
        if($result2){
            $sql = "select * from subjectdb where Activity_ID = $id;";
            $result2 = $conn->query($sql);
            foreach($result2 as $r){
                $sid = $r['Subject_ID'];
            }
            $sid = $conn->real_escape_string($sid);
            $sql = "insert into labdb(Subject_ID, Lab_name) values($sid, '".$class['title']."');";
            $conn->query($sql);
